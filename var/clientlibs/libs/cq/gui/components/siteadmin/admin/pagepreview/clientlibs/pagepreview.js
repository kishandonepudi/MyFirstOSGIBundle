...
window.CQ = window.CQ || {};
window.CQ.Siteadmin = window.CQ.Siteadmin || {};
...
window.CQ.Siteadmin.PagePreview = function () {
    ...
    this.sourcePatn = null;
    this.$frame = null;
    this.callback = null;

    this.extractDataURL = function (canvas) {
    ...
    this.renderToSVG = function () {
        ...
        var body = this.$frame.contents().find('body');
        var html2obj = html2canvas (body, options);
    };

    this.generatePreview = function (path, dest, cl) {
        if (this.debug_mode) {
        ...
        this.sourcePath = path + "/jcr:content/image";
        this.uploadFile = dest;

        if (cl) this.callback = cl;

        var self = this;
        this.$frame[0].src = Granite.HTTP.externalize(path+".html?wcmmode=disabled");
...
$(document).fipo("tap.cq-siteadmin-admin-actions-makepreview-activator", "click.cq-siteadmin-admin-actions-makepreview-activator", ".cq-siteadmin-admin-actions-makepreview-activator", function () {
    var path_list = [];
    ...
    var generatePage = function (donegen) {
        ...
        if (path) {
            ...
            $at.removeClass ("foundation-selections-item");
            $at.append ('<div style="position:absolute;bottom:10px;right:10px;z-index:9999" class="spinner large"></div>');
