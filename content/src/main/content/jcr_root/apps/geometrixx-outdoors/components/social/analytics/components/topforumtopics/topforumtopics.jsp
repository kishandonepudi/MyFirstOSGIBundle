...
String forumPath = currentPage.adaptTo(Node.class).getPath() + ".html";
if(currentPage != null && currentStyle != null && currentPage.getAbsoluteParent(currentStyle.get("absParent", 4)) != null)
 forumPath = currentPage.getAbsoluteParent(currentStyle.get("absParent", 4)).getPath() + "/forum.html";
...
if(reportDataJSON != null && reportDataJSON.has("eventdata.category")) {
    ...
    for (int i=0;i<topics.length();i++){
        ...
        if(!category.getString("name").equals("forum")) continue;
    ...
    for (int i=0;i<topics.length();i++){
        ...
        if(category == null || !category.equals("forum")) continue;
        String topicPath = topic.getString("name");
        if(resourceResolver.getResource(topicPath) == null) continue;
        result.add(topicPath);
    }
...
<div class="title clearfix">
    ...
    <a href="<%= forumPath %>"><%= i18n.get("view all +") %></a>
</div>
...
<% for(String topicPath: result){ 
    ...
    if (topicResource == null){
        log.debug("top forum topic not found {}", topicPath);
        continue;
    }
    final Post topic = topicResource.adaptTo(Post.class);
    if (topic == null){
        log.debug("top forum topic is not a post {}", topicPath);
        continue;
    }
